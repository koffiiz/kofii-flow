...
**`product.compare_at_price` is a minimum, not a variant's price — never fall back to it once a variant is in hand.** It is the lowest compare-at across every variant, so it is set whenever *any* one of them is discounted. `kf-price` used to resolve `target.compare_at_price | default: product.compare_at_price`, which meant a full-price variant borrowed a discounted sibling's compare-at: pick the size that is not reduced and it still drew a strikethrough and the on-sale colour. The fallback is now taken only when no `variant` was passed at all, where it is correct — a card showing a whole product wants the product's own figures.
...
**Scroll may drive state; it must never drive scroll.** Three sections now read scroll position — the scrolling story, the lookbook tour and the scroll video — and none of them calls `scrollTo`, intercepts the wheel or fakes momentum. The page scrolls at its own speed, `position: sticky` does the pinning, and JavaScript only writes attributes and custom properties. That is what keeps keyboard paging, trackpads, find-in-page and screen readers working, and it is not negotiable.

**A trim on a scrubbed video has to be resolved in the browser, not in Liquid.** The merchant types seconds; the duration those seconds must be clamped against is only known once `loadedmetadata` fires, and Liquid never sees it. `kf-scroll-video.js` settles it once in `resolveTrim()` and everything downstream works in `startTime + progress * span` — including the Theme Editor's block-select preview, which would otherwise jump to a frame outside the trimmed stretch. Captions need no change at all: they are keyed to scroll progress rather than absolute time, so "appears at 50%" means halfway through whatever stretch is actually playing.

Every degenerate trim degrades to the full video rather than pinning one frame for three screens of scroll — inverted, identical, non-numeric, negative and past-the-end values are all verified to yield a positive span.

**Scroll input is discrete; smooth motion comes from easing toward it, not following it.** A wheel notch is a jump of tens of pixels, so anything pinned rigidly to scroll inherits that staircase. `kf-scroll-video.js` eases the displayed position toward the scroll position instead, which is most of what "smooth" means for a scrubbed video, and the amount is a merchant setting (0 follows exactly).

The easing is **frame-rate independent**: a fixed fraction per frame would settle twice as fast on a 120Hz display as on a 60Hz one, so the retention is raised to the power of elapsed-over-one-frame. Measured: 60Hz settles in 367ms over 22 frames, 120Hz in 350ms over 42 — same wall-clock, double the frames. A long frame is clamped so a backgrounded tab does not jump the whole way on return, and the loop stops the moment it settles rather than spinning.

Note this slightly weakens the WCAG 2.2 SC 2.2.2 argument and the comment says so: motion no longer stops the *instant* scrolling stops, but when the ease settles — 1.2s at the heaviest setting, far inside the five-second threshold, and it cannot restart without another scroll.

**Playback that is not scroll-derived has to be given back.** The scroll video optionally plays on for a moment each time scrolling stops, because a perfectly frozen frame reads as a stall rather than a pause. That offset must decay to zero the moment scrolling resumes: left to accumulate, ten stops at a third of a second each would leave the video three seconds ahead of where the scroll says it is, and the end of the scroll would stop being the end of the video. It decays with the same easing factor as everything else, which also hides it behind the scroll's own forward movement, and it is clamped to the trimmed end so playing on at the last frame cannot overshoot. Verified: reaches its cap and stops rather than running forever, returns to exactly zero on scroll, and measures zero drift after ten stop/scroll cycles.

**A scrubbed video needs one seek in flight, never a queue.** Each `currentTime` write starts a decode, and a scroll fires many times per frame, so assigning on every event builds a backlog and the picture visibly lags the finger. `kf-scroll-video.js` keeps a single pending target that later positions overwrite, and starts the next seek only once `seeked` fires — so the video tracks where the scroll *is*, not where it has been. Measured: 40 rapid positions collapsed to 2 actual seeks, landing on the latest. Sub-frame moves (below ~33ms) are skipped outright, since they cost a decode and return an identical picture.

Two limits on that section are content and platform facts rather than bugs, and both are surfaced in the schema rather than left to look like faults: **external video cannot be scrubbed at all** (YouTube and Vimeo expose no frame-accurate seek without their own SDK, which rule 8 forbids), and **smooth scrubbing is a property of the source file** — browsers seek between keyframes, so a standard export steps rather than glides. The effect is withdrawn under reduced motion, and below 750px unless the merchant opts in — falling back to an ordinary player with the captions stacked in reading order. A picture moving in lockstep with scroll is exactly the coupling WCAG 2.3.3 addresses, so reduced motion never turns it on rather than turning it on and taking it away: both pinning blocks are gated on `prefers-reduced-motion: no-preference`.

**Every switchable property in that section is a custom property, declared once on the element that owns it with the FALLBACK as its default.** Pinning reassigns the switches; it does not restate the rules. That is what stops the pinned and stacked layouts becoming two parallel stylesheets that drift — and it is why no-JavaScript and reduced motion need no undo block at all. Colour belongs in that set too: pinning flips caption text to the background colour so it reads over the video, and applying that outside the pinned blocks painted background-on-background in the stacked layout, which is invisible rather than merely wrong. There is no darkening overlay — legibility comes from a two-offset text shadow tuned to the glyphs, which keeps the picture undimmed.

**Inherited colour does not reach anything that declares its own.** Three parts of a caption set a colour before it can inherit one: every heading (`h1`–`h6` and `.kf-h*` carry `color: var(--kf-color-heading)` in `kf-base.css`), rich-text links (`--kf-color-accent`), and the plain button (`--kf-btn-fg`, declared on `.kf-button--plain` itself, so it can only be overridden ON the button at higher specificity — the custom-property rule above). Each therefore needs its own switch, or a pinned caption turns white around a heading still painted the scheme's dark heading colour.

Give each switch the value it would have had anyway as its default — `--sv-button-color` mirrors `.kf-button--plain` — rather than pointing several at one shared value. Pointing the button at the link colour made the stacked caption's button accent-coloured, which nobody asked for and the pinned state hid.

**A scroll-scrubbed video needs a runtime escape hatch, not just a CSS one.** `preload` is advisory and Safari on a phone routinely ignores it: the file never loads, `loadedmetadata` never fires, and there is nothing to seek in. Two consequences had to be designed for rather than assumed away — the controls are removed only once a duration has actually arrived (stripping them earlier leaves a dead player on any browser that never loads the file), and `fail()` sets `data-scrub-failed` after a timeout so the section unpins instead of holding one frame for three screens of scroll. `prime()` also plays the video muted and pauses it in the same breath, because a muted inline video may autoplay without a gesture and that is what forces the load.

**That attribute PREVENTS pinning; it does not undo it.** The pinned selectors carry `:not([data-scrub-failed])`. An undo block was tried first and quietly lost on specificity — `.kf-scrub[data-scrub-failed]` at (0,2,0) against the pinned rules at (0,3,0) and above — so captions stayed white-on-white after the section had already given up. Preventing the switch costs nothing and cannot be out-specified.

**Measure a pinned region against the stage, never `window.innerHeight`.** On mobile the two disagree by design: the region is sized in `svh`, the viewport with browser chrome showing, while `innerHeight` grows as that chrome hides mid-scroll. Deriving the scroll travel from `innerHeight` shrinks it during the gesture and makes the progress jump. Verified: the midpoint stays at the same frame when the stage height changes underneath it.

**Hiding an overlaid caption needs `visibility`, not opacity alone.** An `opacity: 0` element is still in the accessibility tree and still collects tab stops, so every caption would be announced at once and a link inside an invisible one would be focusable. `visibility` is discrete, so it is transitioned with a delay out and instantly in — which is why the two transitions are declared separately. Verified: a link inside a hidden caption cannot take focus; the same link in the active caption can.
...
**`kf-bundle.js` is the only place the theme formats money in JavaScript, and it is a deliberate, bounded exception.** The cart rule below — no money arithmetic in JavaScript, ever — still holds everywhere else, and holds absolutely in the cart. It was relaxed for the bundle's running total only after the alternative proved worse in use: a total that sat still while the shopper changed a size reads as broken.

Four things bound it, and all four must survive any change:

- **Liquid renders the opening total.** The server sums the variants each item shows as selected and formats them with the same `money` filter as every other price, so the figure is exact before the script runs and stays exact if it never does.
- **The format is the shop's own.** `shop.money_format`, or `money_with_currency_format` when `settings.currency_code_enabled` is on — the same choice `kf-price` makes — is handed over as a string and the amount substituted into it. No currency convention is invented. All seven Shopify placeholders are covered and verified: comma, period, apostrophe and space thousands separators, with and without decimals.
- **It fails to the server figure.** An unrecognised placeholder, an empty format or a missing attribute writes nothing at all, leaving Liquid's total standing. A stale-but-correct figure beats a confidently wrong one.
- **Nothing downstream reads it.** It is display only: the cart is still asked to state the real total, and the add request carries variant ids alone.

Per-item prices are still never computed. Each variant gets a full `kf-price` render, hidden until its variant is chosen — going through the shared snippet rather than emitting `{{ v.price | money }}` directly is what carries the compare-at strikethrough, the on-sale colour and unit pricing across a variant change. Gate that per-variant rendering on `price_varies` **or** `compare_at_price_varies`: gating on price alone misses the common sale shape where every variant costs the same and only some are discounted, and the item then shows one static price that never updates.
...
- Sections: the rest of the marketing library (comparison table, stats, image reveal).
