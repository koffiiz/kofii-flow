...
`npm run validate` enforces all of the above, plus the range rules that also block an upload: `min < max`, `(max - min)` divisible by `step`, at least 3 and at most 101 steps, the default inside the range **and landing on a step**. The last two are the ones that look fine and still get rejected — a 1-to-2 range, or a default of 68 in a 40/90/5 range.
...
- **No QR code on the gift card.** Shopify does not render one server-side and there is no image endpoint to point an <img> at, so every theme that shows one bundles a QR library. That would break rule 8, so the code is set large, monospaced and selectable instead. Adding it means accepting a dependency — decide that deliberately.
